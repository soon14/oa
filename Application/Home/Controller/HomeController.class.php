...
class HomeController extends Controller {
    ...
    protected function _page($model, $map, $sort = '', $join = [], $field = '*', $page = true) {
        //排序字段 默认为主键名
        ...
        //取得满足条件的记录数
        $map['gender'] = ["neq", 0];
        $count = $count_model ->join($join)-> where($map) -> count();
        ...
        if ($count > 0) {
            ...
            //分页查询数据
            if($page){
                $vo_list = $model ->join($join)-> where($map) -> order($sort) -> limit($p -> firstRow . ',' . $p -> listRows) ->field($field)-> select();
                $p -> parameter = $this -> _search($model);
            }else{
                $vo_list = $model ->join($join)-> where($map) -> order($sort) -> field($field)-> select();
            }
            //分页显示
            $page = $p -> show();
            if ($vo_list) {
