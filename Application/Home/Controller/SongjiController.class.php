...
class SongjiController extends HomeController {
	protected $config = array('app_type' => 'common', 'read' => 'allocate,accept,reject,save_log');
		...
		switch ($fid) {
			...
			case 'dept' :
				...
				if ($auth['admin']) {
					...
					$task_list = M("SongjiLog") -> where($where_log) -> getField('task_id', TRUE);
					if (empty($task_list)) {
						$where['_string'] = '1=2';
					} else {
			...
			case 'no_assign' :
				...
				$assign_list = M("Songji") -> getField('id', true);
	...
	public function read($id) {
		...
		$task_log = M("SongjiLog") -> where($where_log) -> select();
		...
		$task_accept = M("SongjiLog") -> where($where_accept) -> find();
		...
		if ($this -> config['auth']['admin']) {
			...
			$task_dept_accept = M("SongjiLog") -> where($where_dept_accept) -> find();
			if ($task_dept_accept) {
				$this -> assign('is_accept', 1);
		...
		$task_working = M("SongjiLog") -> where($where_working) -> find();
	...
	function accept($task_id) {
		if (IS_POST) {
			M("Songji") -> where("id=$task_id") -> setField('executor', get_user_name() . "|" . get_user_id());
			M("Songji") -> where("id=$task_id") -> setField('status', 10);
	...
	function allocate($task_id) {
		if (IS_POST) {
			M("Songji") -> where("id=$task_id") -> setField('executor', get_user_name() . "|" . get_user_id());
			M("Songji") -> where("id=$task_id") -> setField('status', 20);

			$data['task_id'] = I(task_id);
			$user_id=I('user_id');
			$data['executor'] =$user_id;
			$data['executor_name'] = get_user_name($user_id);
			$data['transactor'] = get_user_id();
			$data['transactor_name'] = get_user_name();
			$data['status'] = 20;
			...
			$menu = array();
			$menu = M("User") -> where('is_del=0') -> field('id,name') -> select();
	...
	public function save_log($id) {
		...
		$task_id = M("SongjiLog") -> where("id=$task_log_id") -> getField('task_id');
		...
		if ($status >= 20) {
			...
			$total_count = M("SongjiLog") -> where($where_total_count) -> count();

			$where_finish_count['task_id'] = array('eq', $task_id);
			$where_finish_count['status'] = array('egt', 20);
			$finish_count = M("SongjiLog") -> where($where_finish_count) -> count();

			if ($total_count == $finish_count) {
				M("Songji") -> where("id=$task_id") -> setField('status', 30);

				$user_id = M('Songji') -> where("id=$task_id") -> getField('user_id');

				$task = M("Songji") -> where("id=$task_id") -> find();
				...
				$push_data['url'] = U('Songji/read', "id={$task['id']}&return_url&Songji/index");
		...
		if ($status = 21) {
			...
			D('Songji') -> forword($task_id, $forword_executor);
		}
		if ($status = 22) {
	...
	private function _add_to_schedule($task_id) {
		$info = M("Songji") -> where("id=$task_id") -> find();
		$data['name'] = $info['name'];
	...
	function _send_mail_finish($task_id, $executor) {
		...
		$transactor_name = M("SongjiLog") -> where("task_id=$task_id") -> getField('id,transactor_name');
		...
		$body .= "点击查看任务详情：http://" . $_SERVER['SERVER_NAME'] . U('Songji/read', 'id=' . $info['id']) . "</br>";
		$body .= "霞湖世家，感谢有您！</br>";
