...
class SongjiModel extends CommonModel {
	// 自动验证设置
	protected $_validate = array( array('passenger', 'require', '乘客必须', 1), array('depart_location', 'require', '出发地点必须'),array('depart_time', 'require', '出发时间必须') );

	function _before_insert(&$data, $options) {
		$sql = "SELECT CONCAT(year(now()),'-',LPAD(count(*)+1,4,0)) task_no FROM `" . $this -> tablePrefix . "songji` WHERE 1 and year(FROM_UNIXTIME(create_time))>=year(now())";
