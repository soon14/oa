...
class  UdfSalesModel extends CommonModel {

	function get_sumary($start_date, $end_date) {
		$where['sales_date'] = array( array('egt', $start_date), array('elt', $end_date));
	...
	function get_target($start_date, $end_date) {
		...
		$list = M('UdfTarget') -> where($where) -> field('shop_no,sum(sales_target) sum') -> group('shop_no') -> select();
		foreach ($list as $key => $val) {
			$new[$val['shop_no']] = $val['sum'];
		}
		return $new;
